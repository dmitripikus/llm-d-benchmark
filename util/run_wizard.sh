...
k=$(type -p oc || type -p kubectl || error_exit 1 "Please install at least one of kubectl or oc. You should run setup/install_deps.sh")
...
prompt="Please select PVC for benchmark workload results
${header}"
prompt2="A new PVC will be created. Please enter a new PVC name"
  ...
  readarray -t classes < <(
    ...
    $k -n "${LLMDBENCH_VLLM_HARNESS_NAMESPACE}" get storageclasses \
      ...
      sed -n -f <(cat <<SED
        # keep header
        1         {p;}
        # remove none for non default
        /<none>/  {s///;H;}
        # mark default, save
        /true$/   {s//(default)/p;}
        # print default as last line
        $         {x;s/.//p;}
SED
      ) || true
  )
...
# # Persistent Volume Claim where model and cache will be stored (must be different from LLMDBENCH_HARNESS_PVC_NAME)
# model_pvcs=()
# workload_pattern="^${LLMDBENCH_HARNESS_PVC_NAME:-###}($|\t| )"
# model_pattern="^${LLMDBENCH_VLLM_COMMON_PVC_NAME:-###}($|\t| )"
# for pvc in "${pvcs[@]:1}"; do
...
#     :   # skip
#   elif [[ "${pvc}" =~ ${model_pattern} ]]; then
#     model_pvcs+=("${pvc} (current value)")
#   else
#     model_pvcs+=("${pvc}")
#   fi
# done
# prompt="Please select another PVC for model and cache
# ${header}"
# export_var LLMDBENCH_VLLM_COMMON_PVC_NAME $(choose_var "${prompt}" "${model_pvcs[@]}") ||
#   error_exit 1 "No available PVCs for LLMDBENCH_VLLM_COMMON_PVC_NAME"
