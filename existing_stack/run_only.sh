...
EOF
  ...
  echo ${control_kubectl} wait --for=condition=Ready=True statefulset/${pod_name} -n ${harness_namespace} --timeout="${KUBECTL_TIMEOUT}s"

  ${control_kubectl} wait --for=condition=Ready=True statefulset/${pod_name} -n ${harness_namespace} --timeout="${KUBECTL_TIMEOUT}s"
  if [[ $? != 0 ]]; then
